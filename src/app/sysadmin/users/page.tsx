    ...
    <div style={{ backgroundColor: currentThemeStyles.mainBg, minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
          ...
          <nav style={{ backgroundColor: 'rgba(30, 41, 59, 0.92)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(251, 191, 36, 0.3)', padding: '1rem 0', position: 'relative', zIndex: 10 }}>
            <div style={{ maxWidth: '1600px', margin: '0 auto', padding: '0 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><UsersIcon width={24} height={24} color="#3b82f6" /><h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#f1f5f9', margin: 0 }}>User Management</h1></div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <ThemeSwitcher />
                <Link href="/sysadmin/users/create" style={{ backgroundColor: '#3b82f6', color: 'white', padding: '0.75rem 1.5rem', borderRadius: '0.5rem', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: '500' }}><PlusIcon width={20} height={20} />Create User</Link>
              </div>
            </div>
          </nav>
          ...
          <div style={{ maxWidth: '1600px', margin: '0 auto', padding: '2rem 1.5rem', position: 'relative', zIndex: 10 }}>
            {stats && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
                <div style={{ backgroundColor: currentThemeStyles.glassPanelBg, padding: '1.5rem', borderRadius: '1rem', boxShadow: currentThemeStyles.glassPanelShadow, backdropFilter: 'blur(12px)', border: currentThemeStyles.glassPanelBorder }}><h3 style={{ color: '#3b82f6', fontSize: '2rem', fontWeight: 'bold', margin: 0 }}>{stats.totalUsers.toLocaleString()}</h3><p style={{ color: currentThemeStyles.textSecondary, margin: '0.5rem 0 0 0' }}>Total Users</p></div>
                <div style={{ backgroundColor: currentThemeStyles.glassPanelBg, padding: '1.5rem', borderRadius: '1rem', boxShadow: currentThemeStyles.glassPanelShadow, backdropFilter: 'blur(12px)', border: currentThemeStyles.glassPanelBorder }}><h3 style={{ color: '#10b981', fontSize: '2rem', fontWeight: 'bold', margin: 0 }}>{stats.activeUsers.toLocaleString()}</h3><p style={{ color: currentThemeStyles.textSecondary, margin: '0.5rem 0 0 0' }}>Active Users</p></div>
                <div style={{ backgroundColor: currentThemeStyles.glassPanelBg, padding: '1.5rem', borderRadius: '1rem', boxShadow: currentThemeStyles.glassPanelShadow, backdropFilter: 'blur(12px)', border: currentThemeStyles.glassPanelBorder }}><h3 style={{ color: '#f59e0b', fontSize: '2rem', fontWeight: 'bold', margin: 0 }}>{(stats.byRole?.client || 0).toLocaleString()}</h3><p style={{ color: currentThemeStyles.textSecondary, margin: '0.5rem 0 0 0' }}>Clients</p></div>
                <div style={{ backgroundColor: currentThemeStyles.glassPanelBg, padding: '1.5rem', borderRadius: '1rem', boxShadow: currentThemeStyles.glassPanelShadow, backdropFilter: 'blur(12px)', border: currentThemeStyles.glassPanelBorder }}><h3 style={{ color: '#8b5cf6', fontSize: '2rem', fontWeight: 'bold', margin: 0 }}>{stats.recentRegistrations.toLocaleString()}</h3><p style={{ color: currentThemeStyles.textSecondary, margin: '0.5rem 0 0 0' }}>Recent (30 days)</p></div>
              </div>
            )}
            ...
            <div style={{ backgroundColor: currentThemeStyles.glassPanelBg, padding: '1.5rem', borderRadius: '1rem', boxShadow: currentThemeStyles.glassPanelShadow, backdropFilter: 'blur(12px)', border: currentThemeStyles.glassPanelBorder, marginBottom: '2rem' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center', marginBottom: '1rem' }}>
                <div style={{ position: 'relative', flex: '1 1 300px' }}><MagnifyingGlassIcon width={20} height={20} color="#94a3b8" style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)' }} /><input type="text" placeholder="Search users by name, email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ width: '100%', padding: '0.75rem 1rem 0.75rem 3rem', borderRadius: '0.5rem', border: currentThemeStyles.inputBorder, backgroundColor: currentThemeStyles.inputBg, color: currentThemeStyles.textPrimary, fontSize: '1rem', boxSizing: 'border-box', outline: 'none' }} /></div>
                <select value={filterRole} onChange={(e) => setFilterRole(e.target.value)} style={{ padding: '0.75rem 1rem', borderRadius: '0.5rem', border: currentThemeStyles.inputBorder, backgroundColor: currentThemeStyles.inputBg, color: currentThemeStyles.textPrimary, fontSize: '1rem', outline: 'none' }}><option value="all">All Roles</option><option value="client">Client</option><option value="customer_service">Customer Service</option><option value="route_admin">Route Admin</option><option value="company_admin">Company Admin</option><option value="system_admin">System Admin</option></select>
                <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} style={{ padding: '0.75rem 1rem', borderRadius: '0.5rem', border: currentThemeStyles.inputBorder, backgroundColor: currentThemeStyles.inputBg, color: currentThemeStyles.textPrimary, fontSize: '1rem', outline: 'none' }}><option value="all">All Status</option><option value="active">Active</option><option value="inactive">Inactive</option></select>
              </div>
              {selectedUsers.length > 0 && (<div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}><span style={{ color: currentThemeStyles.textSecondary, fontSize: '0.875rem' }}>{selectedUsers.length} selected</span><button onClick={() => handleBulkAction('activate')} style={{ backgroundColor: '#10b981', color: 'white', padding: '0.5rem 1rem', borderRadius: '0.375rem', border: 'none', cursor: 'pointer', fontSize: '0.875rem' }}>Activate</button><button onClick={() => handleBulkAction('deactivate')} style={{ backgroundColor: '#f59e0b', color: 'white', padding: '0.5rem 1rem', borderRadius: '0.375rem', border: 'none', cursor: 'pointer', fontSize: '0.875rem' }}>Deactivate</button><button onClick={() => handleBulkAction('delete')} style={{ backgroundColor: '#ef4444', color: 'white', padding: '0.5rem 1rem', borderRadius: '0.375rem', border: 'none', cursor: 'pointer', fontSize: '0.875rem' }}>Delete</button></div>)}
            </div>
            ...
            <div style={{ backgroundColor: currentThemeStyles.glassPanelBg, padding: 0, borderRadius: '1rem', boxShadow: currentThemeStyles.glassPanelShadow, backdropFilter: 'blur(12px)', border: currentThemeStyles.glassPanelBorder, overflow: 'hidden' }}>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead><tr style={{ backgroundColor: currentThemeStyles.tableHeaderBg, borderBottom: `1px solid ${currentThemeStyles.inputBorder}` }}><th style={{ padding: '1rem', textAlign: 'left', color: currentThemeStyles.textPrimary, fontWeight: '600' }}><input type="checkbox" checked={selectedUsers.length === users.length && users.length > 0} onChange={handleSelectAll} style={{ accentColor: '#3b82f6' }} /></th><th style={{ padding: '1rem', textAlign: 'left', color: currentThemeStyles.textPrimary, fontWeight: '600' }}>User</th><th style={{ padding: '1rem', textAlign: 'left', color: currentThemeStyles.textPrimary, fontWeight: '600' }}>Role</th><th style={{ padding: '1rem', textAlign: 'left', color: currentThemeStyles.textPrimary, fontWeight: '600' }}>Status</th><th style={{ padding: '1rem', textAlign: 'left', color: currentThemeStyles.textPrimary, fontWeight: '600' }}>Created</th><th style={{ padding: '1rem', textAlign: 'left', color: currentThemeStyles.textPrimary, fontWeight: '600' }}>Last Login</th><th style={{ padding: '1rem', textAlign: 'left', color: currentThemeStyles.textPrimary, fontWeight: '600' }}>Actions</th></tr></thead>
                  <tbody>
                    {users.map((user, index) => (
                      <tr key={user._id} style={{ borderBottom: index < users.length - 1 ? `1px solid ${currentThemeStyles.inputBorder}` : 'none', backgroundColor: selectedUsers.includes(user._id) ? 'rgba(59, 130, 246, 0.2)' : 'transparent' }}>
                        <td style={{ padding: '1rem' }}><input type="checkbox" checked={selectedUsers.includes(user._id)} onChange={() => handleSelectUser(user._id)} style={{ accentColor: '#3b82f6' }} /></td>
                        <td style={{ padding: '1rem' }}><div><div style={{ color: currentThemeStyles.textPrimary, fontWeight: '500' }}>{user.name}</div><div style={{ color: currentThemeStyles.textSecondary, fontSize: '0.875rem' }}>{user.email}</div>{user.company && (<div style={{ color: currentThemeStyles.textSecondary, fontSize: '0.75rem' }}>{user.company}</div>)}</div></td>
                        <td style={{ padding: '1rem' }}><div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><span style={{ color: getRoleColor(user.role) }}>{getRoleIcon(user.role)}</span><span style={{ color: getRoleColor(user.role), fontWeight: '500' }}>{getRoleLabel(user.role)}</span></div></td>
                        <td style={{ padding: '1rem' }}><button onClick={() => handleToggleUserStatus(user._id, user.isActive)} style={{ backgroundColor: user.isActive ? '#10b981' : '#6b7280', color: 'white', padding: '0.25rem 0.75rem', borderRadius: '0.25rem', fontSize: '0.75rem', fontWeight: '500', border: 'none', cursor: 'pointer' }}>{user.isActive ? 'Active' : 'Inactive'}</button></td>
                        <td style={{ padding: '1rem', color: currentThemeStyles.textSecondary }}>{formatDate(user.createdAt)}</td>
                        <td style={{ padding: '1rem', color: currentThemeStyles.textSecondary }}>{user.lastLogin ? formatDateTime(user.lastLogin) : 'Never'}</td>
                        <td style={{ padding: '1rem' }}><div style={{ display: 'flex', gap: '0.5rem' }}><Link href={`/sysadmin/users/${user._id}`} style={{ color: '#3b82f6', textDecoration: 'none', padding: '0.25rem' }}><EyeIcon width={16} height={16} /></Link><Link href={`/sysadmin/users/${user._id}/edit`} style={{ color: '#f59e0b', textDecoration: 'none', padding: '0.25rem' }}><PencilIcon width={16} height={16} /></Link><button onClick={() => { setUserToDelete(user._id); setShowDeleteModal(true); }} style={{ color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', padding: '0.25rem' }}><TrashIcon width={16} height={16} /></button></div></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '2rem', color: currentThemeStyles.textSecondary }}>
              <div>Showing {users.length} of {pagination.totalUsers} users (Page {pagination.currentPage} of {pagination.totalPages})</div>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <button onClick={() => handlePageChange(pagination.currentPage - 1)} disabled={!pagination.hasPrev} style={{ backgroundColor: pagination.hasPrev ? currentThemeStyles.inputBg : currentThemeStyles.quickActionBg, color: pagination.hasPrev ? currentThemeStyles.textPrimary : currentThemeStyles.textMuted, padding: '0.5rem', border: currentThemeStyles.inputBorder, borderRadius: '0.375rem', cursor: pagination.hasPrev ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center' }}><ChevronLeftIcon width={16} height={16} /></button>
                <span style={{ padding: '0.5rem 1rem', backgroundColor: currentThemeStyles.inputBg, borderRadius: '0.375rem', color: currentThemeStyles.textPrimary }}>{pagination.currentPage}</span>
                <button onClick={() => handlePageChange(pagination.currentPage + 1)} disabled={!pagination.hasNext} style={{ backgroundColor: pagination.hasNext ? currentThemeStyles.inputBg : currentThemeStyles.quickActionBg, color: pagination.hasNext ? currentThemeStyles.textPrimary : currentThemeStyles.textMuted, padding: '0.5rem', border: currentThemeStyles.inputBorder, borderRadius: '0.375rem', cursor: pagination.hasNext ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center' }}><ChevronRightIcon width={16} height={16} /></button>
              </div>
            </div>
          </div>
        </>
      ...
      {showDeleteModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(8px)' }}>
          <div style={{ backgroundColor: currentThemeStyles.glassPanelBg, padding: '1.5rem', borderRadius: '1rem', boxShadow: currentThemeStyles.glassPanelShadow, backdropFilter: 'blur(12px)', border: currentThemeStyles.glassPanelBorder, maxWidth: '400px', width: '90%' }}>
            <h3 style={{ color: currentThemeStyles.textPrimary, marginBottom: '1rem' }}>Confirm Deletion</h3>
            <p style={{ color: currentThemeStyles.textSecondary, marginBottom: '2rem' }}>Are you sure you want to delete this user? This action cannot be undone.</p>
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowDeleteModal(false)} disabled={deleting} style={{ backgroundColor: currentThemeStyles.quickActionBg, color: currentThemeStyles.textPrimary, padding: '0.5rem 1rem', border: currentThemeStyles.inputBorder, borderRadius: '0.5rem', cursor: deleting ? 'not-allowed' : 'pointer', opacity: deleting ? 0.7 : 1 }}>Cancel</button>
